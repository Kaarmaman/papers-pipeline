...
5. If you want the five full analyses, enter an LLM-compatible API key in `LLM_API_KEY`, plus `LLM_BASE_URL` and `LLM_MODEL` if needed. TradingAgents-compatible aliases are also accepted: `NVIDIA_API_KEY`, `TRADINGAGENTS_LLM_BACKEND_URL`, and `TRADINGAGENTS_DEEP_THINK_LLM`. The key is read at runtime and never written to MongoDB or the report. Without it, the report explicitly uses metadata-only analysis and does not invent methodology or results.
