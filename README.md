...
5. If you want the five full analyses, enter an LLM-compatible API key in `LLM_API_KEY`, plus `LLM_BASE_URL` and `LLM_MODEL` if needed. These names are provider-neutral; use the endpoint and model identifier for your chosen provider. The key is read at runtime and never written to MongoDB or the report. Without it, the report explicitly uses metadata-only analysis and does not invent methodology or results.
